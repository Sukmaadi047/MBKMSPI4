        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if ($row['add_card'] == 0) {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    $sql = "SELECT nim FROM users WHERE nim=? AND id NOT like ?";
                    $result = mysqli_stmt_init($conn);
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        echo "SQL_Error";
                        exit();
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            $sql="UPDATE users SET username=?, nim=?, gender=?, email=?, user_date=CURDATE(), device_uid=?, device_dep=?, add_card=1 WHERE id=?";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                ...
                                $end = microtime(true);
                                $waktu = $end - $start;
                                $sql1 = "UPDATE users SET respon_insert=? WHERE nim=? ";
                                $result1 = mysqli_stmt_init($conn);
                                    if (!mysqli_stmt_prepare($result1, $sql1)) {
                                        echo "SQL_Error_Respon_Add";
                                        exit();
                                    }
                                    else{
                                        mysqli_stmt_bind_param($result1, "ds", $waktu, $Number);
                            ...
                            echo "Nomer induk sudah terdaftar!";
                            exit();
                        }
                    }
                }
    ...
    if (!mysqli_stmt_prepare($result, $sql)) {
      ...
      exit();
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email)) {
                                ...
                                $sql="UPDATE users SET username=?, nim=?, gender=?, email=?, device_uid=?, device_dep=? WHERE id=?";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
            ...
            echo "Tidak ada user yang di update!";
            exit();
        }
    }
}
